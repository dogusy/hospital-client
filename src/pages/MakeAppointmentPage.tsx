...
const MakeAppointmentPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    var isValid = Validate.validatePatientAppointment(formData);
    if (!isValid) {
      return;
    }
    if (
      formData.username.trim().length === 0 &&
      formData.tc.trim().length !== 0
    ) {
      dispatch(fetchAppointmentsByTC(formData.tc))
        .then(() => navigate("/patienthome"))
        .catch(() => toast("Bir sorun oluştu."));
    } else {
            ...
            >
              ...
              >
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <InputLabel id="hospitalTypeLabel">
                      Hastahane Tipi
                    </InputLabel>
                    <Select
