...
export const HospitalDetailsTable: React.FC = () => {
  const { hospitalName, hospitalAddress, type } = useSelector(
    (state: RootState) => state.hospital
  );
  const [isHospitalDetailEditModalOpen, setHospitalDetailEditModalOpen] =
  ...
  const handleFormSubmit = (data: Hospital): void => {
    dispatch(updateHospitalDetails(data))
      .then(() => {
  ...
  function createData(id: number, name: String, address: String, type: String) {
    return { id, name, address, type };
  }

  const rows = [createData(1, hospitalName, hospitalAddress, type)];
            ...
            <TableRow key={row.id}>
              ...
              <TableCell>
                <Button
                  variant="outlined"
                  color="success"
                  onClick={handleOpenHospitalDetailEditModal}
                >
                  Değiştir
                </Button>
              </TableCell>
